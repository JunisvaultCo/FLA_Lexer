...
class LexerTests extends munit.FunSuite{
  ...
  test("Test lexer: simple concat (2p)"){
    val spec =
      """A: a;
        ...
        #DEF: def;
        #""".stripMargin('#')
    val result1 = Lexer(spec).lex("a")
    assert(result1 == Right(List(("a", "A"))), result1)
    val result2 = Lexer(spec).lex("aa")
    assert(result2 == Right(List(("a", "A"), ("a", "A"))), result2)
  ...
  test("Test lexer: space and zeros char (5p)"){
    ...
    val result2 = Lexer(spec).lex("aa")
    assert(result2 == Right(List(("a", "smallA"), ("a", "smallA"))), result2)
  ...
  test("Test lexer: lexer for a real language (20p)"){
    ...
    def transform(lexRes: Either[String,List[(String,String)]]) : List[String] = {
      lexRes match {
        case Left(a) => throw new Exception("Parsing cannot fail here, revise you configuration " + a)
        case Right(l) => l.map(elem => elem._2)
      }
    }

    val results = words.map(word => {
      transform(Lexer(spec).lex(word))
    })
